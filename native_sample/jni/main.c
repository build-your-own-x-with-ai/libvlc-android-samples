...
#define SAMPLE_URL "https://download.blender.org/peach/bigbuckbunny_movies/BigBuckBunny_640x360.m4v"
...
PlayUrl(struct context *p_ctx, const char *psz_url)
{
    ...
    if (psz_url[0] == '/')
        p_m = libvlc_media_new_path(psz_url);
    else
        p_m = libvlc_media_new_location(psz_url);
    if (p_m == NULL)
        return false;
...
{
    libvlc_media_player_stop_async(p_ctx->p_mp);
}
